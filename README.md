...
[vignette](vignettes/ScisorWiz-vignette.pdf).

The package has only been tested on CentOS x86\_64 and macOS.
...
the **ScisorWiz\_AllInfo** function can be running by providing a path
to the input file (AllInfo.gz), the gene name (“*Snap25*”), the
clustering parameter (1 for intron chain), and a mismatch file
(optional) containing SNV information (Snap25.mismatches.txt.gz) using
...
[vignette](vignettes/ScisorWiz-vignette.pdf). To access it, run
